...
function newVtId() { return 'vt-' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6) }
function newVvId() { return 'vv-' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6) }
function newPvId() { return 'pv-' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6) }
    ...
    <div className="space-y-5 border-t border-slate-100 pt-5">
        ...
        <div>
          ...
          {variantTypes.length > 0 && (
            <p className="text-[10px] text-slate-400 mt-0.5">
              {variantTypes.length} {lang === 'id' ? 'tipe' : 'type'}
              {variantTypes.length > 1 ? (lang === 'id' ? ' (misal: Warna, Ukuran)' : ' (e.g. Color, Size)') : ''}
              {' → '}
              {variantTypes.reduce((s, vt) => s + vt.values.length, 0)} {lang === 'id' ? 'nilai' : 'values'}
              {' → '}
              {combinations.length} {lang === 'id' ? 'kombinasi' : 'combinations'}
            </p>
          )}
        </div>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={lang === 'id' ? 'Nama tipe varian (misal: Warna, Ukuran)...' : 'Variant type name (e.g. Color, Size)...'}
      ...
      {variantTypes.length > 0 && (
        <div>
          <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3">
            {lang === 'id' ? 'Langkah 1 — Tipe Varian' : 'Step 1 — Variant Types'}
            <span className="ml-1.5 text-slate-300 font-normal normal-case">({lang === 'id' ? 'misal: Warna, Ukuran' : 'e.g. Color, Size'})</span>
          </h4>
            ...
            <div key={vt.id} className="bg-slate-50 rounded-xl border border-slate-100 p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-brand-primary text-white text-[10px] font-bold flex items-center justify-center">T</span>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
              {lang === 'id' ? 'Langkah 2 — Kombinasi Varian' : 'Step 2 — Variant Combinations'}
              <span className="ml-1.5 text-slate-300 font-normal normal-case">({combinations.length})</span>
            </h4>
            <div className="flex items-center gap-2">
              <input
                value={bulkPrice}
              ...
              <button type="button" onClick={applyBulkPrice} className="px-2 py-1 text-[10px] font-bold text-slate-500 bg-slate-100 hover:bg-slate-200 rounded-lg transition-all">
                {lang === 'id' ? 'Semua Harga Sama' : 'All Same Price'}
              </button>
              <input
                value={bulkStock}
