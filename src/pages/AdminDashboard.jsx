...
export default function AdminDashboard() {
  ...
  const sidebarLinks = [
    ...
    { to: '/admin/add', label_id: 'Tambah Produk', label_en: 'Add Product', icon: '➕', isLink: true },
    { to: '/admin/import', label_id: 'Import Harga', label_en: 'Price Import', icon: '📥', isLink: true },
    { to: '/admin/revenue', label_id: 'Pendapatan', label_en: 'Revenue', icon: '💰', isLink: true },
  ]
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      <MetaTags title={`Admin — Dunia Pancing`} />
            ...
            <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
              ...
              <div className="p-2 space-y-0.5">
                {sidebarLinks.map(link => {
                  if (link.isLink) {
                    return (
                      <Link key={link.key || link.to} to={link.to}
                        ...
                        <tr key={p.id} className="border-t border-slate-50 hover:bg-slate-50/50 transition-colors">
                          {editingId === p.id ? (
                            ...
                            <>
                              ...
                              <td className="px-4 py-3 text-right">
                                <button onClick={() => startEdit(p)} className="text-brand-primary font-bold text-[11px] hover:text-sky-700 mr-3">{lang === 'id' ? 'Edit' : 'Edit'}</button>
                                {confirmDelete === p.id ? (
                                  <>
                      ...
                      <div key={cat.key} className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        {editingCat !== cat.key && (
                          <div className="flex items-center gap-2 shrink-0">
                            <button onClick={() => startEditCat(cat)} className="px-3 py-1.5 text-[11px] font-bold text-brand-primary hover:bg-sky-50 rounded-lg transition-all">{lang === 'id' ? 'Edit' : 'Edit'}</button>
